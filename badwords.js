...
export function containsBadWords(text) {
    ...
    // 1. Pasamos el nombre a minúsculas
    // 2. Le quitamos los acentos (ej: é -> e) para evitar "péné"
    // 3. Le quitamos todos los espacios y guiones para evitar "p e n e" o "pe-ne"
    let textoLimpio = text.toLowerCase()
                          .normalize("NFD")
                          .replace(/[\u0300-\u036f]/g, "")
                          .replace(/[\s\-_.]+/g, ''); 
    
    // Comprobamos si alguna palabra prohibida está escondida dentro del nombre
    for (let i = 0; i < forbiddenWords.length; i++) {
        if (textoLimpio.includes(forbiddenWords[i])) {
            return true; // Cazado
        }
    }
